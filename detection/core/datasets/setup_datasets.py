...
def setup_all_datasets(dataset_dir, image_root_corruption_prefix=None):
    ...
    setup_voc_completely_annotation_dataset(dataset_dir)
    setup_coco_dataset(
        dataset_dir,
        image_root_corruption_prefix=image_root_corruption_prefix)
    setup_coco_ood_dataset(dataset_dir)
    setup_coco_id_dataset(dataset_dir)
    setup_coco_mixed_dataset(dataset_dir)
    setup_coco_extended_ood_dataset(dataset_dir)
...
def setup_voc_completely_annotation_dataset(dataset_dir):
    ...
    register_coco_instances(
        ...
        test_image_dir)
    MetadataCatalog.get(
        "voc_completely_annotation_pretest").thing_classes = metadata.VOC_THING_CLASSES
    MetadataCatalog.get(
        "voc_completely_annotation_pretest").thing_dataset_id_to_contiguous_id = metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID
...
def setup_coco_id_dataset(dataset_dir):
    test_image_dir = os.path.join(dataset_dir, 'val2017')
    test_json_annotations = os.path.join(
        dataset_dir, 'annotations', 'instances_val2017_onlyID.json')
    ...
    register_coco_instances(
        ...
        test_image_dir)
    MetadataCatalog.get(
        "coco_id_val").thing_classes = metadata.VOC_THING_CLASSES
    MetadataCatalog.get(
...
def setup_coco_mixed_dataset(dataset_dir):
    ...
    register_coco_instances(
        ...
        test_image_dir)
    MetadataCatalog.get(
        "coco_mixed_val").thing_classes = metadata.VOC_THING_CLASSES
    MetadataCatalog.get(
        "coco_mixed_val").thing_dataset_id_to_contiguous_id = metadata.VOC_THING_DATASET_ID_TO_CONTIGUOUS_ID
...
def setup_coco_extended_ood_dataset(dataset_dir):
    ...
    register_coco_instances(
        ...
        test_image_dir)
    # import ipdb; ipdb.set_trace()
    metadata_COCO_THING_CLASSES = metadata.COCO_THING_CLASSES
